...
class BuildApk:
    ...
    def package_cus_app_parm(self,env):
        param = {}
        if "1" == env:
            ...
            param["BUILD_CHANNAL"] = "preview_mix_nat"
            param["BUILD_FLAG"] = "ldygo"
            param["BUILD_MESSAGE"] = "jenkins构建-性能环境APP"
            param["BUILD_TYPE"] = "Debug"
            param["tinkerEnabled"] = "false"
        elif "3" == env:
            param["BUILD_CHANNAL"] = "production"
            param["BUILD_FLAG"] = "ldygo"
            param["BUILD_MESSAGE"] = "jenkins构建-预发布环境APP"
            param["BUILD_TYPE"] = "Debug"
            param["tinkerEnabled"] = "false"
        return param
    ...
    def job_build(self):
        ...
        env_chose = input("选择构建的环境：1.测试环境；2.性能环境；3.预生产\n")
        job_name = ""
        if chose =="androidAPP-4.0":
            ...
            if not my_job.is_queued_or_running():
                ...
                if env_chose not in ("1","2","3"):
                    log.info("不打包apk！")
                else:
                    param = self.package_cus_app_parm(env_chose)
                    try:
