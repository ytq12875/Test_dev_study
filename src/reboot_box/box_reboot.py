...
class BoxReboot:
    ...
    def thread_stop(self,th_id, env_choose, box_name):
        rb = RebootBox(th_id, env_choose, box_name)
        try:
            rb.stop_box()
        except Exception as e:
            print(e)
    ...
    def do_stop_box(self):
        while True:
            env = input("请输入你要停止服务的环境序号：\n1.===>融合测试环境\n2.===>预发布环境\n其他输入===>退出\n")
            if str(env) in ("1", "2"):
                if str(env) == "1":
                    env_choose = "uat"
                else:
                    env_choose = "pre"
                box_name = input("请输入你要停止的BOX名：多个BOX请以英文的逗号隔开。\n ").strip()
                boxlist = box_name.split(",")
                threads = []
                nloops = range(len(boxlist))
                for i in nloops:  # 根据输入的box进行多线程的创建
                    t = threading.Thread(target=self.thread_stop, args=(i, env_choose, boxlist[i]))
                    threads.append(t)
                for i in nloops:  # start threads 此处并不会执行线程，而是将任务分发到每个线程，同步线程。等同步完成后再开始执行start方法
                    threads[i].start()
                for i in nloops:  # jion()方法等待线程完成
